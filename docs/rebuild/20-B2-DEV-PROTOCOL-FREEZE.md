...
* fingerprints the **harness** itself -- the three run scripts and the two declared seed files, by content
  digest -- so the frozen command surface is pinned and not merely named;
...
and the fingerprint table is regenerated on the clean execution revision (§12 records the values).

The frozen instrument's own fingerprints -- closure commit, reference-artifact digest, calibration and
mutation artifacts, the 53-source tree digest and the 694-node test inventory -- are recorded in
...
`scripts/f0_manifest_selfcheck.py` (see §12 for the count after this revision's additions).
