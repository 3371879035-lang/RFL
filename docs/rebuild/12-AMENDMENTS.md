...
$$\boxed{\text{G1 (refactor equivalence): for } W \in \{W_{\text{pre}},\ W^{cal}_{D_Q},\ W^{cal}_X\},
\quad \Sigma_{\text{suffix}}\bigl(\texttt{continuation}(\texttt{START}, \texttt{initial\_control}(z_0))\bigr)
= \Sigma_{\text{suffix}}\bigl(\text{ordinary rollout}\bigr)}$$

$$\boxed{\text{G2 (decomposition equivalence): for } W = W^{cal}_P,\
\Sigma_{\text{suffix}}\bigl(\texttt{continuation}(\texttt{START}, \texttt{ControlState}(z^{\text{in-force}}, 0))\bigr)
= \Sigma_{\text{suffix}}\bigl(\text{ordinary rollout from the post-commit state onward}\bigr),
\qquad z^{\text{in-force}} = C_P^{L}(z_0)}$$
...
`BLIND` verdict cannot be attributed to the candidate.
