...
d_A\bigr)}, \qquad A \in \{D_Q, X, P\}}$$

where the direction field is **status-typed**, not a sign that occasionally holds a status:

$$\boxed{d_A \in \{+1,\ -1,\ \texttt{DIRECTION\_UNRESOLVED}\}}$$
...
* $d_A$ — the direction field, **status-typed from the start**:

$$\boxed{d_A \in \{+1,\ -1,\ \texttt{DIRECTION\_UNRESOLVED}\}}$$

  A numeric $d_A$ is the **independently declared expected direction of the behavioural loss**, a
  constant A87 states from the canary's construction, before any candidate is projected and before
  anything is measured; `DIRECTION_UNRESOLVED` is the explicit no-direction state. There is exactly
  **one** direction variable. An earlier draft carried $d_A$ in the fixture schema *and* a second
  $s_A^{\text{expected}}$ in the gate, so schema and contract named different things; the gate now
  reads the fixture's own field:

$$\text{if } d_A \in \{-1, +1\}:\quad
\operatorname{sign}\!\left[V_W\bigl(g_U(\xi_A^*)\bigr) -
V_{W+\Delta W_A^{\text{cal}}}\bigl(g_U(\xi_A^*)\bigr)\right] = d_A$$

$$\text{if } d_A = \texttt{DIRECTION\_UNRESOLVED}:\quad
\text{no direction equality test is applied}$$

  so `semantic canary -> declared direction` happens before any candidate, while
...
### 74.3 The detection protocol, and its closed status space

For a candidate $U$ and architecture $A$, with $u^*_{A,U} := g_U(\xi_A^*)$, a cell's status is
exactly one of:

1. **coverage.** $u^*_{A,U} \neq \varnothing$: the candidate can represent the witness at all. A
   candidate that cannot has failed structurally, and this is a different defect from a metric that
...
$$\boxed{
\operatorname{status}(U,A)=
\begin{cases}
\texttt{BLIND}, & \neg\text{coverage} \lor \neg\text{detection}\\[1mm]
\texttt{DIRECTION\_UNRESOLVED}, & \text{coverage} \land \text{detection} \land
  d_A = \texttt{DIRECTION\_UNRESOLVED}\\[1mm]
\texttt{STRUCTURAL\_PASS}, & \text{coverage} \land \text{detection} \land d_A \in \{-1,+1\}
  \land \text{measured} = d_A\\[1mm]
\texttt{DIRECTION\_FAIL}, & \text{coverage} \land \text{detection} \land d_A \in \{-1,+1\}
  \land \text{measured} \neq d_A
\end{cases}}$$

The cases are ordered and their clauses are disjoint, so the function is single-valued: the four
labels are mutually exclusive and exhaustive, and

$$\boxed{\texttt{BLIND} \text{ takes precedence over the direction statuses}}$$

because if the unit cannot represent the witness, or the measurement cannot see an effect, there is
nothing left for a direction to be right or wrong about. An earlier draft listed the four statuses as
independent conditions, which made `BLIND` and `DIRECTION_UNRESOLVED` simultaneously true of a
candidate that failed coverage while A87 could declare no sign — contradicting "exactly one of four".

$$\boxed{\texttt{STRUCTURAL\_REJECT} = \{\texttt{BLIND},\ \texttt{DIRECTION\_FAIL}\}}, \qquad
\texttt{DIRECTION\_UNRESOLVED} \notin \texttt{STRUCTURAL\_REJECT}$$
...
### 74.4 The $P$ canary, and the three possible conclusions
...
the edge, and — with type, state and CRN correct and the value genuinely measured — is
`BLIND` for $P$, by the coverage or detection clause. This is the cell that decides the shape of the
answer.

The screening has **three** possible conclusions, and they are defined on the status matrix:

$$\boxed{\exists U\ \forall A:\ \texttt{STRUCTURAL\_PASS}(U, A)
\;\Longrightarrow\; \texttt{UNIFIED\_SURVIVES}}$$

$$\boxed{\forall U\ \exists A:\ \texttt{STRUCTURAL\_REJECT}(U, A)
\;\Longrightarrow\; \texttt{ALL\_PREREG\_UNIFIED\_REJECTED}}$$

$$\boxed{\text{otherwise} \;\Longrightarrow\; \texttt{SCREENING\_INCONCLUSIVE}}$$

The third is not a formality. With $U_1$ `BLIND` for $P$ and $U_2$ `DIRECTION_UNRESOLVED` for $P$, no
candidate passes all three and no candidate is structurally rejected either, so neither of the first
two rules fires — and the honest reading is that the screening **could not adjudicate**:

$$\boxed{\texttt{DIRECTION\_UNRESOLVED} \neq \texttt{STRUCTURAL\_PASS}, \qquad
\neq \texttt{STRUCTURAL\_REJECT}}$$

`SCREENING_INCONCLUSIVE` must not be allowed to become an architecture-specific trigger by omission:
the trigger is `ALL_PREREG_UNIFIED_REJECTED`, and an unresolved cell is not a rejection. What it
licenses is a narrower response — amend A86 or extend A87 so that the direction becomes declarable,
and screen again.

Two further bounds stand. `ALL_PREREG_UNIFIED_REJECTED` rejects the **preregistered** candidates and
is not the claim that no unified domain can exist, which would need $\mathcal U^{\text{pre}}_{\text{unified}}$
complete for admissible unified designs, neither proved nor claimed. And since
...
survivor fails them, the architecture-specific branch must remain enterable. $U_3$ is therefore not
conditional on a structural rejection alone.
...
nothing in A79 or A85. Its output, when the calibration is eventually run, is a structural-screening status matrix
...
## 75. A87 — the concrete injection fixtures and the nominal screening protocol

A86 §74.6 fixes what this section owes, and A86 §74.2 fixes the schema:

$$\mathcal C_A = \bigl(W_{\text{pre}},\ \Delta W_A^{\text{cal}},\ \xi_A^*,\ d_A\bigr),
\qquad A \in \{D_Q, X, P\}$$

**No measurement was consulted to choose anything below.** Every fixture is selected by a
**deterministic rule over a pre-declared structurally valid set**, so that a canary which later
screens badly is a **scientific result** rather than an invitation to pick a better-looking address:

$$\boxed{\text{selection} = \text{the canonical (lexicographically first) element of a declared
structurally valid set}}$$

Static work only: this section reads frozen contracts and uses already-generated reference artifacts.
...
$\xi_A^*$ is the **candidate-independent semantic witness** of A86 §74.2 -- a situation of the
environment, not a representation of one. It is therefore not itself a cell: each witness **induces two
cells**, $(U_1, A)$ and $(U_2, A)$, one per candidate, and a cell's content is the witness mapped through
that candidate's own projection $g_U$. Keeping the witness candidate-independent is what makes the same
situation the subject of screening for every candidate -- candidates are screened, not injections chosen
to suit one of them -- so the projection table below is a table of induced cells, not a redefinition of
$\xi_A^*$. Six induced cells, and A86 §74.6 requires their images frozen **before** the run:

| cell | $g_{U_1}(\xi_A^*) = (s, z, m)$ | $g_{U_2}(\xi_A^*) = (\kappa, \text{tape}, \text{base option})$ |
|---|---|---|
| $D_Q$ | $(\texttt{State}(0,2,0,0,0),\ z{=}1,\ m{=}0)$ | $(0,\ (0,0,0),\ 1)$ |
| $X$ | $(\texttt{State}(0,2,0,0,0),\ z{=}1,\ m{=}0)$ | $(0,\ (0,0,0),\ 1)$ |
| $P$ | $(\texttt{State}(0,2,0,0,0),\ z{=}0,\ m{=}0)$ | $(0,\ (0,0,0),\ 0)$ |

$\texttt{State}(0,2,0,0,0) = (x{=}0, y{=}2, t{=}0, \kappa{=}0, \phi{=}0)$ and the tape triple is
$(\text{phase}, \texttt{error\_flag}, \texttt{cause\_rank})$. Both coordinates are **pre-update**
projections: they are built from the scene and the frozen reference, never from anything the arm
produced.

The $P \times U_1$ image is the load-bearing one:

$$\boxed{g_{U_1}(\xi_P^*) = (\texttt{START},\ z = z_0,\ m = 0), \qquad z_0 = \min \texttt{option\_ids}() = 0}$$

$z_0$ is the value the scene **proposes**, not $z_1 = C_P^{L}(z_0)$, the option the arm's process law
puts in force. Handing the continuation the post-write option would make $U_1$'s blindness to $P$ an
artefact of the harness instead of a property of the candidate, because the entire content of

$$U_1 \text{ fixes } z \text{ before the arm-specific process write can affect evaluation}$$

is that the $z$ sealed into the unit's identity is the one that exists **independently of the arm
under test**. The projection is therefore taken before the commit, by rule, as a frozen part of the
cell rather than a choice left to the fixture builder.

The degeneracy note of §75.8 is not in tension with this table: $\lvert A_z \rvert$ decides whether a
*decision-channel injection* is admissible, while legality of an image decides whether it is a
*member of the candidate's domain*. $(\texttt{START}, z{=}0, m{=}0)$ is legal -- it is in
$\mathcal X_D$ by construction -- with a single admissible action, which is exactly why the $P$
canary is injected through the process channel rather than through a decision edit.
...
**The $U_1$ exogenous lift is frozen rather than chosen by the implementation.** $U_1$'s identity is
$(s,z,m)$, so $V_W^{meas}$ must be a function of $(s,z,m)$ alone, while the continuation also consumes
an exogenous continuation. A86 §74.1 refuses to omit `error_flag` or `cause_rank` from $U_2$'s tape
without a proof that neither can affect $V_W$, and A87 will not apply the opposite standard inside
$U_1$. Instead of enlarging the candidate's ontology to $(s,z,m,e,r)$ -- which would change what the
unit *is* -- one deterministic lift is frozen:

$$\boxed{\lambda_{U_1}(s,z,m) := \texttt{SemanticTape}\bigl(\text{phase} = s.\phi,\
\texttt{error\_flag} = 0,\ \texttt{cause\_rank} = 0\bigr)}$$

applied identically to the pre and the post measurement. It is a **measurement-protocol input, not unit
identity**: it never enters $\mathcal X_D$, never appears in a cell's name, and leaves $U_1$ and its
cardinality untouched. What it removes is the implementation's freedom to complete the same unit with a
different legal tape per run, which would make $u \mapsto V$ a relation instead of a function. Without
it, the two §75.6 clauses -- `tape.phase = s.phi` and a shared complete assignment -- pin down only the
phase and pin it down only *within* a pair, so the pairing would be consistent while the map was not
defined.
...
standing next to a red kernel or B1 suite is a broken environment with five passing assertions, not a
closed instrument.
...
### 75.6 The $U_1$ continuation contract

To measure $V_W$ at a decision context the instrument is

$$\boxed{(W,\ s,\ z,\ m,\ \text{exogenous continuation}) \;\longrightarrow\;
\text{roll from that } State/ControlState \text{ to terminal or } H}$$

with five binding properties. The first two are domain conditions: they say which inputs the
instrument may be handed at all.

**(i) The entry must be a legal decision context.**

$$\boxed{\texttt{is\_decision\_context}(s, z, m) = \texttt{True}}$$

judged by the **frozen** enumerator in `env/domain.py`, which checks types first and then values
(`is_true_int`, `is_strict_decision_state`, `M_DOMAIN`, `Z_DOMAIN`). The instrument may not carry its
own copy of the domain: a continuation that redefined legality would be measuring a different
environment, and a contract validated by the code that consumes it is not validated. An arbitrary
`State`/`ControlState` pair is therefore **not** accepted -- membership in $\mathcal X_D$ is required,
which is also what makes the image table of §75.2 well defined.

**(ii) The exogenous continuation must be consistent with the entry.**

$$\boxed{\texttt{tape.phase} = s.\phi, \qquad \kappa = s.\kappa}$$

The kernel **derives** the episode's phase from the tape rather than sampling it separately, so a
continuation carrying $\texttt{State}(\phi{=}0)$ together with a tape whose phase is $3$ describes a
suffix the frozen environment cannot produce. The tape is drawn once per episode, so this is a single
equality, not a per-step one. The $\kappa$ agreement is the same principle applied to the other
exogenous coordinate, stated rather than assumed because both the state and the setup carry it. One
**complete** tape assignment $(\text{phase}, \texttt{error\_flag}, \texttt{cause\_rank})$ is shared by
the pre and the post measurement (A85 §73.2.3), so a difference between $V_W$ and $V_{W + \Delta W}$
can only come from $\Delta W$.

**(iii) One simulator, not two.** The episode loop is factored into a shared **continuation core**: a
full rollout is `construct initial state/control` $\to$ core, and a $U_1$ measurement is `given
state/control` $\to$ the *same* core. Two implementations that agree are still two implementations,
and gates G1--G3 below are what distinguish agreement from identity.

**(iv) No re-execution of $C_P^{L}$.** A continuation from $(s,z,m)$ takes $z$ as **already in force**
and does not apply the process commit again. This is not a convenience: $U_1$'s structural blindness
to $P$ *consists* in fixing $z$ at its entry, so a continuation that re-ran $C_P^{L}$ would
manufacture the sensitivity the screening is supposed to test for, and could only ever "detect" it.

**(v) One complete tape assignment for pre and post**, as fixed in (ii); listed separately because it
is the property that makes a difference interpretable.

**The equivalence gates.** A87@8c16deb stated a single equality,
`continuation(START, initial_control(z)) == ordinary rollout`, which is **false as written** for an
arbitrary $W$: the ordinary rollout runs
$z^{\text{proposal}} \to C_P^{L} \to z^{\text{in-force}} \to \texttt{initial\_control}$, while the
continuation deliberately does not run $C_P^{L}$, so for a $W$ with $C_P^{L}(z_0) = z_1$ the two begin
from different options and *should* differ. It is replaced by two equivalence gates and a poison gate.

"Equal" is not left to the implementer either. The frozen comparison object is the **behaviour-bearing
suffix signature**

$$\boxed{\Sigma_{\text{suffix}} = (\texttt{steps},\ \texttt{outcome},\ \texttt{final control})}$$

compared **object by object and field by field for exact equality**. $V_W$ is *derived* from
$\Sigma_{\text{suffix}}$; it is not what an equivalence gate compares, because two internally different
simulators can return the same scalar, and a gate that compared only the scalar would not support the
claim in (iii).

**Which $W$ the equivalence gates run on is frozen as well.** "For $W$ with no process override" is
not yet a test set, and the set matters most for the two candidates whose edits live *inside* the
continuation. A wrapper that always used the reference decision provider and the identity controller
would pass G5 (baseline only), G4 (healthy traces) and G3 (no recommit), and would then report
$D_Q \times U_1$ and $X \times U_1$ as `BLIND` -- instrument blindness presented as candidate
blindness, which is precisely what A85 §73.2.5--§73.2.6 forbid. So the witness set is frozen:

$$\boxed{\mathcal W_{\text{gate}} := \{W_{\text{pre}},\ W^{cal}_{D_Q},\ W^{cal}_X,\ W^{cal}_P\},
\qquad W^{cal}_A := W_{\text{pre}} + \Delta W_A^{cal}\}}$$

$$\boxed{\text{G1 (refactor equivalence): } \forall A \in \{D_Q, X\},\
\forall W \in \{W_{\text{pre}},\ W^{cal}_A\}:\quad
\Sigma_{\text{suffix}}\Bigl(\texttt{continuation}_W\bigl(g_{U_1}(\xi_A^*),\
\lambda_{U_1}(g_{U_1}(\xi_A^*))\bigr)\Bigr)
= \Sigma_{\text{suffix}}\Bigl(\texttt{ordinary\_rollout}_W\bigl(g_{U_2}(\xi_A^*)\bigr)\Bigr)}$$

$$\boxed{\text{G2 (decomposition equivalence): } W = W^{cal}_P,\quad
g_{U_2}(\xi_P^*) = (0, (0,0,0), 0),\quad z^{\text{in-force}} = C_P^{L}(0) = 1,\quad
\Sigma_{\text{suffix}}\Bigl(\texttt{continuation}_{W}\bigl(\texttt{START},\
\texttt{ControlState}(z^{\text{in-force}}, 0),\ \lambda_{U_1}(\texttt{START}, z^{\text{in-force}}, 0)\bigr)\Bigr)
= \Sigma_{\text{suffix}}\Bigl(\texttt{ordinary\_rollout}_W\bigl(g_{U_2}(\xi_P^*)\bigr)\ \text{from the post-commit state onward}\Bigr)}$$

The ordinary rollout runs the frozen P scene first, so **its own** commit produces
$z^{\text{in-force}} = C_P^{L}(0) = 1$; the continuation is then entered at
$(\texttt{START}, 1, 0)$ -- a legal decision context, since $(s,z,m) = (\texttt{START},1,0) \in
\mathcal X_D$ -- with the lift $\lambda_{U_1}(\texttt{START},1,0) = \text{phase } 0, 0, 0$, which is
exactly the frozen P scene's own tape $(0,0,0)$. The two sides of G2 therefore share both the scene and
the exogenous assignment, and nothing in the gate has to interpret a bare $z_0$.

$$\boxed{\text{G3 ($P$-poison): }
\Sigma_{\text{suffix}}\bigl(\texttt{continuation}\bigr)^{\text{poison}}
= \Sigma_{\text{suffix}}\bigl(\texttt{continuation}\bigr)^{\text{clean}},
\quad\text{and}\quad
\texttt{option\_in\_force}^{\text{poison}} \neq \texttt{option\_in\_force}^{\text{clean}}
\text{ on the paired ordinary rollout}}$$

G1 is the healthy case and is what licenses replacing the episode entry with the core. G2 is the general
case and is what licenses reading a continuation as *the suffix* of an ordinary rollout: without it,
$V_W$ measured by continuation and $V_W$ measured inside a rollout would be two quantities. G3 is what
makes (iv) more than a comment -- a $C_P^{L}$ override is planted whose application would change the
option in force at entry, and the continuation's signature must be **identical** with and without the
poison. Its liveness half is witnessed by the **option in force**, not by a return: the poison's only
purpose is to show that the ordinary entry really does read $C_P^{L}$, and keying liveness to a
difference in $V$ or in the signature would let the gate fail spuriously whenever two different options
happen to share a return.

Running G1 on $W^{cal}_{D_Q}$ and $W^{cal}_X$ is what makes the $D_Q$ and $X$ post-update learner
channels a **precondition of closure** rather than something inferred from a screening result: it shows
that the same post-update learner, read through the continuation and through the ordinary rollout,
produces the same suffix semantics. That is a statement about instrument validity, not a treatment
effect -- it measures no $\Delta V^{meas}_A$ and so takes nothing from the screening -- but without it a
`BLIND` verdict cannot be attributed to the candidate. Binding each $W$ to **its own frozen scene** is what makes
that non-vacuous. Both projections are already frozen in §75.2, so the binding adds no design freedom;
but it is not a notational tidy-up. Run from $\texttt{START}$ with $z_0 = 0$ instead, the $D_Q$ gate
would never reach the row that $\Delta W^{cal}_{D_Q}$ edits, and the $X$ gate would not even be the
calibration situation $\mathcal C_X$ was built on: for $z = 0$ at $\texttt{START}$ the admissible set is
$A_{z=0} = \{3\}$, so a controller-induced $3 \to 4$ would fall outside the learner contract rather
than inside $A_z$ as it is for $z = 1$. In both cases a continuation that ignored the post-update
learner could still have passed, which is the same instrument blindness in a different costume. With
the images bound, $g_{U_1}(\xi_{D_Q}^*) = (\texttt{START}, 1, 0)$ and
$g_{U_2}(\xi_{D_Q}^*) = (0, (0,0,0), 1)$ put $\Delta W^{cal}_{D_Q}$ on the executed path by
construction, and the $X$ gate queries exactly the legal site
$\texttt{ControllerSite}(\texttt{START}, 3)$.

**G4 closes the generality gap.** The contract accepts any $(s,z,m) \in \mathcal X_D$, but G1--G3 all
enter at $\texttt{START}$: a wrapper that reset a non-zero $t$, dropped the passed $m$, or resumed with
the wrong remaining horizon could satisfy all three. So the gate is extended to the interior of the
frozen canary traces:

$$\boxed{\text{G4 (non-start suffix): for every pre-action context } c_t,\ t > 0,
\text{ on the frozen canary scenes' healthy traces, }
\Sigma_{\text{suffix}}\bigl(\texttt{continuation}(c_t)\bigr)
= \Sigma_{\text{suffix}}\bigl(\text{ordinary rollout}[t:]\bigr)}$$

The context set is enumerated from the frozen canary scenes -- no new scenes, no arm, no seed -- and it
must include at least one $t > 0$ with $m = 0$ and, if the frozen healthy traces contain one, at least
...
$$\boxed{\text{G5 (full domain): } \forall (s,z,m) \in \mathcal X_D:\quad
V^{cont}_{W_{\text{pre}}}\bigl(s,z,m;\ \lambda_{U_1}(s,z,m)\bigr) = V^{*}(s,z,m)}$$
...
invented when the continuation is implemented. G5 is stated with $\lambda_{U_1}$ rather than with a bare phase
compatibility clause on purpose: the object it must validate is the **instrument the screening will
actually use**, not a wider continuation that accepts any legal tape completion. Phase compatibility is
then implied by the lift's own definition, since $\lambda_{U_1}$ sets $\text{phase} = s.\phi$. G1, G2 and G4 prove the core is not a second simulator,
...
### 75.7 What is known before the screening

Coverage and detection are **cell-level** and there are $2 \times 3 = 6$ induced cells, so the status of
"the canaries" is not one statement. Before the screening runs:

| cell | coverage | detection | direction |
|---|---|---|---|
| $D_Q \times U_1$ | PASS (static) | not frozen | $d_{D_Q} = +1$ |
| $D_Q \times U_2$ | PASS (static) | not frozen | $d_{D_Q} = +1$ |
| $X \times U_1$ | PASS (static) | not frozen | $d_X = +1$ |
| $X \times U_2$ | PASS (static) | not frozen | $d_X = +1$ |
| $P \times U_1$ | PASS (static) | **false by contract** | $d_P = +1$ |
| $P \times U_2$ | PASS (static) | not frozen | $d_P = +1$ |

**Coverage is frozen PASS for all six, and it is a representation property, not a measurement.** It is
proved against the **frozen candidate-domain contracts**: the $U_1$ images are members of $\mathcal X_D$
by the executable `env/domain.py` predicate, while the $U_2$ images are checked against A86 §74.1's
frozen field surface $(\kappa, \text{full tape}, \text{base option})$ and the frozen domains of those
fields. No executable $U_2$ validator exists yet -- `EvaluationScene` is still a specification-level
object -- so this is a static judgement against a contract, not a validator run, and it must not be
argued from reachability of the canary scene: a structurally reachable scene does not make
$g_U(\xi_A^*)$ a legal member of the candidate's domain, and coverage asks the latter.

**The three directions are declared $+1$, from the frozen exact-reference model.** A86 §74.2 fixes the
convention as the sign of the behavioural **loss**, and A87 declares the direction from a
**preregistered reference prediction**, which is a different object from what the screening measures:
...
$$Q^{*}(\cdot,\ z{=}0,\ m{=}0) = 0.92, \qquad Q^{*}(\cdot,\ z{=}1,\ m{=}0) = 0.88, \qquad
Q^{*}(x^{*},\ 3) = 0.88, \qquad Q^{*}(x^{*},\ 4) = 0.86$$

* **$D_Q$.** The edit lowers the healthy argmax at $x^{*}$ and the provider's read path is an argmax over
  $Q^{\text{eff}}$, so $t = 0$ selects $4$. Time strictly increases, so the edited address is never
  re-encountered and the rest of the episode is the reference policy from $(x^{*}, 4)$, which is the row
  $Q^{*}(x^{*}, 4)$:
  $$\boxed{\Delta V^{ref}_{D_Q} = Q^{*}(x^{*}, 3) - Q^{*}(x^{*}, 4) = +0.02
  \;\Longrightarrow\; d_{D_Q} = +1}$$
* **$X$.** The healthy command is $3$ and the controller site maps it to $4$. Neither action reaches
  $\texttt{WAYPOINT\_Z2}$ at that step, so the automaton is still $m = 0$ afterwards and the post-step
  world and control are exactly the reference successors of action $4$:
  $$\boxed{\Delta V^{ref}_{X} = 0.88 - 0.86 = +0.02 \;\Longrightarrow\; d_X = +1}$$
* **$P$.** The pre evaluation is the unit entered at $(\texttt{START}, z_0, m{=}0)$ and the post
  evaluation the same unit with $z_1$ in force, so the two terms are the frozen solver's option values
  at $\texttt{START}$:
...
The derivation is the frozen exact solve plus two structural facts -- the argmax read path, and
`t`-strict monotonicity making an edited context unrepeatable. What it assumes is that the canary
scenes' frozen tape assignments realize the nominal path, so that no hazard draw separates a realized
return from the value the solver row names. That assumption is a property of the frozen hazard schedule
at $(\kappa, \phi) = (0, 0)$ and of no arm, and it is why A86 calls $d_A$ a **declared** direction
rather than a measured one; it is the only premise here that is not arithmetic.

**Detection is not frozen by A87, and is not inherited from B1.** B1's mutation self-checks establish
that $\Delta W_A \neq 0$ and that the persistent channel and its read path are mechanically
load-bearing: they license $\Delta W^{\text{cal}}_A$ as a **legal persistent edit** under the frozen
...
here would decide the screening before it runs; detection is exactly what the authorised structural
screening measures, for five of the six cells.
...
code rather than a verified property of the instrument.

**Consequences for the aggregate.** Coverage is PASS on all six cells and every $d_A$ is numeric, so no
cell can reach `DIRECTION_UNRESOLVED`: each cell is either `STRUCTURAL_PASS`, or `BLIND` or a
`DIRECTION_FAIL` and therefore a `STRUCTURAL_REJECT`. A86 §74.3's three-state rule stays frozen; **this
instance excludes its third state**:

$$\boxed{\text{first screening} \in
\{\texttt{UNIFIED\_SURVIVES},\ \texttt{ALL\_PREREG\_UNIFIED\_REJECTED}\}}$$

`UNIFIED_SURVIVES` requires a unified candidate to be `STRUCTURAL_PASS` on all three architectures,
which here means $U_2$: detection holding on $D_Q$, $X$ and $P$, with the measured sign agreeing with
$d_A = +1$. $U_1$ cannot survive, because $P \times U_1$ is `BLIND` and `BLIND` is a cell-level
`STRUCTURAL_REJECT`. Otherwise $U_2$ carries at least one rejection and, together with $U_1$'s, the
aggregate is `ALL_PREREG_UNIFIED_REJECTED`. The run therefore decides between survival and rejection of
the unified set -- two outcomes, not three -- and the asymmetry between the two candidates comes from
the projection table rather than from a design that forbids a positive verdict.

Note the vocabulary: `STRUCTURAL_REJECT` is a **cell-level** category while A86 §74.3's aggregate
verdicts are the three above. A87@8c16deb used the two interchangeably; this is the correction.

### 75.8 Review record (history): the draft's DQ and X canaries are structurally degenerate

Recorded as **statically established structural facts**, obtained by reading the frozen kernel and the frozen
...
**Review round 6 (5976dc7).** Two closure holes in the $U_1$ instrument were identified and
closed. $U_1$'s exogenous completion was unspecified, so $u \mapsto V$ was a relation rather than a
function; a deterministic lift $\lambda_{U_1}(s,z,m) = \texttt{SemanticTape}(s.\phi, 0, 0)$, shared by
pre and post and declared a protocol input rather than unit identity, is now frozen (§75.4). And the
equivalence gates were not quantified over any fixed set of $W$, so a wrapper that never read the
post-update $Q_D^L$ or $C_X^L$ could pass G3, G4 and G5 and then report $D_Q \times U_1$ and
$X \times U_1$ as `BLIND`; the witness set $\mathcal W_{\text{gate}}$ and its split between G1 and G2
are frozen (§75.6), making those learner channels a precondition of closure. A stale count of "four
passing assertions" was corrected to five.


**Review round 7 (3871552).** G1 and G2 froze a witness set of $W$ but no scene to run those
$W$ on, and the bare $z_0$ in their statements named $\mathcal C_P$'s base option. Read literally, the
$D_Q$ gate would never reach the row $\Delta W^{cal}_{D_Q}$ edits, and the $X$ gate would not be a legal
$\mathcal C_X$ situation at all, since $A_{z=0} = \{3\}$ at $\texttt{START}$ -- so a continuation that
ignored the post-update learner could still have passed. G1 and G2 are now bound to the frozen
projection images $g_{U_1}(\xi_A^*)$ and $g_{U_2}(\xi_A^*)$, and G5 is stated against
$\lambda_{U_1}(s,z,m)$ so that it validates the instrument the screening uses.


**Promotion.** A87's content was rebuilt onto `rebuild` from the draft line's clean content
rather than cherry-picked: the draft's ancestry is not linear with `rebuild` (§74.6), so the section was
re-applied onto `rebuild`'s frozen tree. The draft line's other deltas -- the B2-4 handoff note and the
two `experiments/v03r` fingerprint updates -- are **not** part of this promotion and remain development
artifacts. Content freeze was approved at draft revision `3ec3b0f`; the status becomes frozen once the
promotion-integrity review confirms that this section is equivalent to that revision and carries nothing
else. One editorial normalisation was applied here: the $\mathcal C_P$ clause in the summary row, which
had been mangled by a blanket replacement during the draft revision, now reads
$C_P^{L}(0) \leftarrow 1$ with base option $z_0 = 0$ and $z_1 = 1$.


**Freeze.** Promotion-integrity review passed on `rebuild@b4b0bba`: `a206041` to `b4b0bba` with a
single changed file, $\S$75's first 598 lines byte-identical to the approved draft `3ec3b0f`, the 11 added
lines being the promotion note above, and `B2-4-DRAFT-HANDOFF.md` absent from `rebuild`. A87 is therefore
**FROZEN** at `b4b0bba`. By §75.5 this authorises implementing the $U_1$ continuation instrument and its
G1--G5 closure, and nothing else: the structural screening stays gated on continuation CLOSED, and no B2
scientific seed, smoke-5, development-32 or V0.4R run is authorised.

## 76. A88 — X-canary domain legality, and the first screening attempt

**Status: FROZEN at `rebuild@3f1f4e1`.** A88 supersedes A87 §75 **only** where the X
fixture is concerned: §75.1's X structurally valid set, §75.2's X site and target, the two X projection
images, the X reference-predicted loss and its declared direction, and G1's binding for X. A86 is not
touched, and A87 stands as the frozen history of what was believed before the defect below was found.

### 76.1 The defect, and why it is a fixture defect

A87 §75.2 froze the X canary as

$$\Delta W^{\text{cal}}_X:\quad
C_X^{L}\bigl(\texttt{ControllerSite}(\texttt{START}, 3)\bigr) \leftarrow 4$$

justified by "$4$ is inside the same $A_z$, so the write is admissible rather than a fault". That
justification is **witness-local**. It holds at $(\texttt{START}, z{=}1, m{=}0)$, where
$A_z(m,s) = \{3,4\}$, but the controller mapping is keyed by $(s, a^{\text{cmd}})$ -- there is no
$(z,m)$ in the key -- so the same mapping is shared by every option context that reaches that state.
For $\texttt{rush}$ at $\texttt{START}$, $A_{z=0}(\texttt{START}) = \{3\}$, so $3 \to 4$ leaves the
option's admissible set and the kernel raises `LearnerContractViolation` by design (A75 §62.3: a
learner baseline may not expand its node's behavioural domain).

That raise is correct behaviour, not an exception to be swallowed, and A86's detection quantifies over
$U$, so the harness may not skip the units. Measured on the frozen domains:

| architecture | $U_1$ units that raise | $U_2$ scenes that raise |
|---|---|---|
| $D_Q$ | $0/13824$ | $0/5760$ |
| $X$ | $2/13824$ -- exactly $(\texttt{START}, 0, 0)$ and $(\texttt{START}, 0, 1)$ | $120/5760$ -- all with base option $0$ |
| $P$ | $0/13824$ | $0/5760$ |

The defect is therefore X-specific and structural: a write target that is legal for one option is not
legal for all of them, and the site key cannot distinguish them.

### 76.2 The rule is strengthened, not the answer chosen

Fixing this after a screening attempt has begun invites the reading that the first canary was replaced
by a friendlier one. So the **rule** is strengthened first, and the fixture is then what the rule
already implies. A87 §75.1's canonical rule is kept verbatim -- *the least element of the declared
structurally valid set* -- and the X set becomes

$$\boxed{\mathcal S_X^{\text{valid}} = \Bigl\{(s, a^{\text{cmd}}, a')\ :\
\begin{array}{l}
s \text{ on the frozen healthy reference path},\\
a^{\text{cmd}} \text{ is the command that path actually sends at } s,\\
a' \neq a^{\text{cmd}},\quad
a' \in \displaystyle\bigcap_{z \in Z,\ m \in \{0,1\}} A_z(m, s)
\end{array}\Bigr\}}$$

The added conjunct is exactly the constraint the old definition lacked: admissibility **for every
option and automaton state that can arrive at the site**, not for the witness's own $(z,m)$.

Enumerating the frozen healthy reference path (base option $1$, $\kappa = 0$, tape $(0,0,0)$, healthy
learner) under this rule:

| site on the path | $a^{\text{cmd}}$ | $\bigcap_{z,m} A_z(m,s)$ | legal $a'$ |
|---|---|---|---|
| $t{=}0$, $\texttt{START}$ | $3$ | $\{3\}$ | none |
| $t{=}1$, $(1,2)$ | $0$ | $\varnothing$ | none |
| $t{=}2$, $(1,1)$ | $3$ | $\{1,3\}$ | $\{1\}$ |
| $t{=}3$, $(2,1)$ | $1$ | $\{3\}$ | $\{3\}$ |
| $t{=}4$, $(2,2)$ | $3$ | $\{3\}$ | none |
| $t{=}5$, $(3,2)$ | $3$ | $\varnothing$ | none |

so the least valid element is the $t{=}2$ site with the unique alternative $a' = 1$. The new fixture
is therefore **derived**, not selected: it is the canonical element of a set defined by a constraint
that the previous definition was missing, and the previous witness is excluded by that constraint
rather than by preference.

### 76.3 The X fixture from A88 on

$$\boxed{s_X^* = \texttt{State}(1,1,t{=}2,\kappa{=}0,\phi{=}0)}$$

$$\boxed{\Delta W^{\text{cal}}_X:\quad
C_X^{L}\bigl(\texttt{ControllerSite}(s_X^*, 3)\bigr) \leftarrow 1}$$

$$\boxed{g_{U_1}(\xi_X^*) = \bigl(\texttt{State}(1,1,2,0,0),\ z{=}1,\ m{=}0\bigr)},
\qquad
\boxed{g_{U_2}(\xi_X^*) = \bigl(0,\ (0,0,0),\ 1\bigr)}$$

The $U_2$ image is unchanged: it is still the same episode-level evaluation scene, and the path visits
$s_X^*$ at $t = 2$. The rewrite is admissible for every $(z,m)$: the intersection is $\{1,3\}$ and the
target is $1$.

**The declared direction is re-derived, not adjusted to taste.** The frozen solver's rows at
$(s_X^*, z{=}1, m{=}0)$ are

$$Q^{*}(1) = 0.88, \qquad Q^{*}(3) = 0.92, \qquad Q^{*}(4) = 0.90, \qquad
\arg\max = 3$$

so the healthy command is still $3$, the executed command becomes $1$, and the edited site cannot recur
because $t$ strictly increases. Hence

$$\boxed{\Delta V^{ref}_X = Q^{*}(s_X^*, 1, 0, 3) - Q^{*}(s_X^*, 1, 0, 1) = 0.92 - 0.88 = +0.04
\;\Longrightarrow\; d_X = +1}$$

under A86 §74.2's convention $\operatorname{sign}[V_W - V_{W + \Delta W}] = d_A$. The direction is
unchanged from A87 and only the magnitude differs ($+0.02 \to +0.04$), which is worth stating plainly:
this correction is not a direction repair. $d_{D_Q} = d_P = +1$ are untouched, and A87 §75.7's
two-state aggregate argument is unaffected because it depended only on all three directions being
numeric.

### 76.4 G1 must be rebound to a suffix

The old X witness sat at $t = 0$, so G1 could compare a continuation against a whole rollout. The new
witness sits at $t = 2$, where a continuation yields a **suffix**, so G1 generalises to

$$\boxed{\forall A \in \{D_Q, X\},\ \forall W \in \{W_{\text{pre}},\ W^{cal}_A\}:\quad
\Sigma_{\text{suffix}}\Bigl(\texttt{continuation}_W\bigl(g_{U_1}(\xi_A^*),\ \lambda_{U_1}(g_{U_1}(\xi_A^*))\bigr)\Bigr)
= \Sigma_{\text{suffix}}\Bigl(\texttt{ordinary\_rollout}_W\bigl(g_{U_2}(\xi_A^*)\bigr)[\,t_A^*\,:\,]\Bigr)}$$

with

$$\boxed{t_{D_Q}^* = 0, \qquad t_X^* = 2}$$

Nothing new is needed to express it: the instrument already exposes `suffix_from`. G2--G5 are
contractually unchanged -- the $P$ fixture is untouched, the $U_2$ image of $X$ is still the same
scene, G4's healthy traces are the same traces, and G5 does not involve any canary.

**Consequence for the closure ledger.** `Continuation = CLOSED` was established against the A87
fixture. Rebinding G1 changes the evidence, so the closure must be **re-earned**:

$$\boxed{\text{Continuation CLOSED under A87} \;\not\Longrightarrow\;
\text{Continuation CLOSED under A87} + \text{A88}}$$

until G1 has been re-run on the new images together with G2--G5 and the kernel/B1 regressions. Until
then the previous closure stands only for the superseded fixture and may not be used as the screening's
precondition.

### 76.5 The first screening attempt, and what it produced

The first authorised screening attempt is recorded here because it happened, and because a later
reader must not have to reconstruct it from a branch:

* the harness formed the closed nominal maps for the pre state and began the post-state maps when the
  X learner raised `LearnerContractViolation` at the units of §76.1;
* **no cell was judged, no status matrix was produced, no aggregate verdict was computed, and no
  evidence artifact was written** -- the harness refuses to judge cells from a partial map, and that
  refusal is the reason the defect surfaced as a stop rather than as a plausible-looking six-cell
  table with $122$ units quietly skipped;
* the partial measurements from that attempt **must not be reused**. The corrected run is from
  scratch, in one process, with no cached maps: a cache would make the recorded domain depend on which
  attempt happened to fill it, which is the same defect as a short-circuit;
* the attempt drew no seed and selected no development-stage quantity.

### 76.6 Two harness corrections found in the same review

Both are completions of frozen contracts rather than new science.

**(a) The zero measured sign.** A86 §74.3's direction clause is a comparison at the witness, and its
measured quantity can be $0$: if $\Delta V(u^*) = 0$ while some $u' \neq u^*$ moves, then detection
holds, the condition fails, and the cell is `DIRECTION_FAIL`. The harness must therefore admit

$$\boxed{\text{measured} \in \{-1, 0, +1\}}, \qquad
\text{status} = \begin{cases}
\texttt{STRUCTURAL\_PASS} & \text{measured} = d_A\\
\texttt{DIRECTION\_FAIL} & \text{measured} \neq d_A
\end{cases}$$

and a regression gate freezes exactly that case, because treating $0$ as a protocol error would
collapse `DIRECTION_FAIL` into a crash and would re-open the `BLIND`-versus-`DIRECTION_FAIL`
distinction §74.3 was corrected for.

**(b) $\mathbb R_{\text{finite}}$.** A85 and A87 freeze $V_W^{meas} : U \to \mathbb R_{\text{finite}}$,
so the closed-map check refuses non-finite values as well as missing and extra units. The kernel
produces finite rewards today; the check exists so that the map is closed in the sense the contract
claims, not merely complete in its keys.

### 76.7 Status and boundary

$\boxed{\text{A88: FROZEN at rebuild@3f1f4e1}} \qquad
\boxed{\text{corrected structural screening: AUTHORISED, from scratch only}}$

A88 authorises nothing by itself. It does not authorise the screening re-run, any B2 scientific seed,
smoke-5, development-32 or V0.4R, and it does not license relaxing A75's learner-domain contract, A86's
detection domain, or A86's status space.


**Freeze and reclosure.** Review passed on `3f1f4e1`: A88's scientific content was approved at
`fc0b494`, and the three freeze-before-run gate closures were verified at `3f1f4e1` -- the X
canonical fixture is selected by an explicit lexicographic minimum over the strengthened valid set,
`aggregate_verdict` fails closed on anything other than the complete $2\times3$ status matrix, and
the $D_Q/P$ legality controls cover both $U_1$ and $U_2$. The branch was a strict fast-forward from
`rebuild@371f44d`, so `rebuild` was advanced directly to `3f1f4e1` with no reconstruction.

The rebound G1 (with $t_{D_Q}^*=0$ and $t_X^*=2$) and G2--G5, together with the existing kernel/B1
regressions, are green on that same reviewed tree. Therefore:

$\boxed{\text{Continuation = CLOSED under A87 + A88}}$

and A87 §75.5's next authorisation now applies:

$\boxed{\text{corrected structural screening re-run = AUTHORISED}}$

The re-run must be from scratch and may not reuse any partial map from the failed first attempt.
This authorises only the seedless A86/A87/A88 screening. B2 scientific seeds, smoke-5,
development-32 and V0.4R remain **NOT AUTHORISED**.


## 77. A89 — $U_2$ qualification and the B2-3 compatibility contract

**Status: FROZEN at `rebuild@6c28fbc`.** A89 records the screening result and specifies how
the sole surviving candidate is to be qualified. It selects nothing: the choice of an unaffected
construction is A79 §67.4's measurement-property decision, which is settled on development seeds and is
**not** authorised here.

### 77.1 The screening result, recorded

$$\boxed{\text{StructuralScreening}@\texttt{4472ec4} \;=\; \texttt{UNIFIED\_SURVIVES}}$$

| unit | $A$ | status | detection | measured | units moved |
|---|---|---|---|---|---|
| $U_1$ | $D_Q$ | `STRUCTURAL_PASS` | true | $+1$ | $1/13824$ |
| $U_1$ | $X$ | `STRUCTURAL_PASS` | true | $+1$ | $4/13824$ |
| $U_1$ | $P$ | `BLIND` | false | $0$ | $0/13824$ |
| $U_2$ | $D_Q$ | `STRUCTURAL_PASS` | true | $+1$ | $120/5760$ |
| $U_2$ | $X$ | `STRUCTURAL_PASS` | true | $+1$ | $120/5760$ |
| $U_2$ | $P$ | `STRUCTURAL_PASS` | true | $+1$ | $1440/5760$ |

The verdict is A86 §74.4's **first** rule, satisfied by $U_2$: $\exists U\ \forall A:
\texttt{STRUCTURAL\_PASS}$. The quantification is existential in the candidate, so $U_1$'s `BLIND` cell
does not bear on it -- and $U_1$ leaves the unified-selection path, because $P \times U_1$ is a
cell-level `STRUCTURAL_REJECT` by the frozen no-recommit contract.

Two properties of the run are load-bearing for what follows. The $\Delta W_P$ map of $U_1$ is
**byte-identical** to the unedited map over all $13824$ units, which is the no-recommit rule confirmed
domain-wide rather than at the witness. And every measured witness loss equals its preregistered
reference prediction ($+0.02$, $+0.04$, $+0.04$), so no direction was repaired after the fact.

$$\boxed{U_2 = \text{the sole surviving preregistered unified candidate}} \qquad
\boxed{U_2 \neq \text{selected}}$$

A86 §74.4's own words govern the inference: a survivor is **admissible, not selected**. It may still
fail the remaining properties, and if it does, the architecture-specific branch must stay enterable.

### 77.2 Why the existing B2-3 objects cannot be pointed at $U_2$

A86 §74.5 anticipated this: if the final unit is not a decision-context type, B2-3's compatibility has
to be closed before anything is built on it. Concretely, today's objects are typed by the old ontology:

* `collateral.py` carries `UnaffectedSet.contexts` and constructs membership from
  `decision_contexts()`; its `state_parity_*` candidates read `c[0].x`, i.e. `State.x`;
* `runner.py`'s pre-update source hardwires `domain=decision_contexts()` and builds touched contexts
  from `address.state`, `address.z`, `address.m`.

None of those fields exists in $U_2 = \mathcal K \times \mathcal T \times \mathcal Z$. Feeding scene
tuples into that machinery would produce code that runs and semantics that are wrong -- the failure mode
this rebuild keeps rejecting:

$$\boxed{\text{old ontology's algorithm} \;+\; \text{new ontology's tuple} \;\neq\; \text{a definition}}$$

So the compatibility closure is a **frozen dependency**, not an implementation detail: the unaffected
region must be defined in the surviving unit's own terms first.

### 77.3 The $U_2$-native ontology

An evaluation scene is a unit

$$u = \bigl(\kappa,\ \text{tape} = (\text{phase}, \texttt{error\_flag}, \texttt{cause\_rank}),\
z_{\text{base}}\bigr) \in U_2$$

and the unaffected region is a **set of such units**. The nominal object is

$$\boxed{\texttt{EvaluationSceneSet}:\quad U_{\text{unaffected}} \subseteq U_2}$$

with `UnaffectedSet.units` rather than `.contexts`. The interface rules, all of them gates rather than
conventions:

* **strict typing.** Every member is a scene whose fields are true integers in the frozen supports
  ($\kappa \in \mathcal K$, phase $\in \text{PHASE\_DOMAIN}$, $e \in \{0,1\}$,
  $r \in [0,60)$, $z_{\text{base}} \in \mathcal Z$). A float or a bool that merely compares equal is
  not a member, for the reason `env/domain.py` already states;
* **closed and definite.** Membership is total on $U_2$: every unit is in or out, with no third answer,
  no duplicate member, and no member outside the domain;
* **non-empty and proper.** $U_{\text{unaffected}} \neq \varnothing$ and
  $U_{\text{unaffected}} \subsetneq U_2$. An empty region makes the metric undefined; the whole domain
  makes it the treatment effect wearing a different name;
* **pre-update and arm-blind.** Membership is computed from frozen objects -- the unit's own fields, and
  for one candidate the *unedited* learner's trajectory -- and from nothing the update or a measurement
  produced;
...
|---|---|
| the credited unit $c$ **at credited-site granularity** -- `DecisionAddress(state, z, m)` on the $D_Q$ channel (A76/A77: `owner_Q(QAddress) = DecisionAddress`), `ControllerSite(state, cmd)` on $X$, the proposal key on $P$ -- declared with the pair before any arm runs | evaluator truth ($Z^{\text{fire}}$, $J^L$, $\Gamma_T^*$, $\Gamma_P^*$, $R^{\text{mech}}$, $R^{\text{rescue}}$, $\pi_{\text{credit}}$) |
...
| the frozen exact solve, for the *correct beforehand* conjunct | the architecture's canary identity as a **source of $c$**: eligibility may not fetch it or branch on it |

A89 freezes **one** eligibility construction, not a family. Registering two would make the refined
candidate set $2 \times 6 = 12$ and the calibration obligations $36$, and the second variant was in any
case ill-defined as posed: a scene that never consults $c$ has no behaviour "at $c$" to judge, so a
conjunct evaluated there is undefined rather than stricter. Both of A79 §67.4's requirements are
therefore carried by **two scene-level conjuncts**:

$$\boxed{E(c) = \bigl\{u \in U_2:\ \neg\textit{Consult}(u, c)\ \land\ H_{\text{pre}}(u)\bigr\}}$$

with, in the frozen vocabulary:

$$\textit{Consult}(u, c) := \text{the } W_{\text{pre}} \text{ episode of } u \text{ consults the credited
address } c$$

$$H_{\text{pre}}(u) := \bigl[\operatorname{outcome}\bigl(W_{\text{pre}}, u\bigr) = \texttt{SUCCESS}\bigr]$$

$\neg\textit{Consult}$ is *unrelated to the credited site*; $H_{\text{pre}}$ is *correct beforehand*, and
it is deliberately **scene-level**: it is a property of the unedited episode's own outcome, evaluable
for every unit, and it does not presuppose that the episode went anywhere near $c$. Both conjuncts are
properties of the unedited behaviour and of the frozen solve, so eligibility stays pre-update,
arm-blind and truth-blind.

**The contract is quantified over the production credited domain.** $E$ is a function of $c$, so its
properties are obligations for every credited unit a runner can present, not only for the three
calibration sites of §77.7:

$$\boxed{\forall c \in \mathcal D^{\text{credit}}_A:\quad E(c) \neq \varnothing, \qquad
E(c) \subsetneq U_2, \qquad \text{membership closed and definite on } U_2}$$

A legitimate $c$ that produced an empty $E(c)$ fails the admissibility gate: it is not skipped, and the
runner may not fall back on a different unit.

**The quantified domain has an executable extent.** "For all $c$" without an enumerator is still prose,
and B1's `AddressDomain` classes say which addresses are *legal*, not which population a runner will
present. So the extents are named:

$$\boxed{\mathcal D^{\text{credit}}_{D_Q} = \{\texttt{DecisionAddress}(s,z,m):\ (s,z,m) \in \mathcal X_D\}
\ \text{filtered by } \texttt{DQ\_DOMAIN.require\_credited}}$$

$$\boxed{\mathcal D^{\text{credit}}_{P} = \{z:\ z \in \texttt{option\_ids}()\}\ \text{filtered by }
\texttt{P\_DOMAIN.require\_credited}}$$

$$\boxed{\mathcal D^{\text{credit}}_{X} = \{\texttt{ControllerSite}(s, a^{\text{cmd}})\ \text{constructed by
the } W_{\text{pre}} \text{ episodes of } U_2\}\ \text{filtered by }
\texttt{X\_DOMAIN.require\_credited} \ \text{and by } \texttt{require\_admissible\_sites(sites, rows)}}$$

$X$ needs **both** filters: `require_credited` checks the site's type and the strict typing of its state
and command, while `require_admissible_sites(sites, rows)` additionally demands a learner-visible row and
$a^{\text{cmd}} \in A_z(m,s)$ -- so `X_DOMAIN` alone is not the credited population. The owner projection
is the domain's own: `DQ_DOMAIN.owner` (the `dq_owner` projection, whose store-level form is `owner_Q`),
and the identity for $X$ and $P$.

**Totality is required per refinement, not only per eligibility.** The six candidates are
$C_i(c) = E(c) \cap S_i$, and $E(c) \neq \varnothing$ does not imply $C_i(c) \neq \varnothing$: a
refinement can be empty at a legitimate credited unit, and an empty set has **no mean**, so
`BehavioralCollateral` would be undefined rather than small. Hence

$$\boxed{\forall c \in \mathcal D^{\text{credit}}_A,\ \forall i:\quad C_i(c) \neq \varnothing}$$

is an **admissibility** requirement, judged seedlessly and before any development work: a refinement that
is empty somewhere is an inadmissible candidate construction, not a coverage statistic. Whether a
non-empty subset is wide enough to be worth reporting is the separate development-stage question, and it
only becomes askable once definedness holds:

$$\boxed{\text{defined} \;\neq\; \text{good enough}}$$ Calibration fixtures and production-domain totality are
**different obligations** -- the first is the eighteen cells below, the second is this quantifier.
...
longer asked to answer "why is this universe unaffected".** Eligibility is **mandatory and frozen**:
there is exactly one construction and it is not a development-stage choice. What remains open is the
refinement -- which slice $S_i$ a report uses -- and that is A79 §67.4's measurement-property decision.

Provenance, not value, is what the rule constrains: eligibility may not **fetch or branch on** a canary
identity, while the calibration harness of §77.7 may hand the same frozen value in as an ordinary
pair-shared credited input $c$ -- the credited site *is* the canary's site there, and saying so is not a
leak.
...
among them. Interpretability comes in **two classes**, and conflating them is what let the first draft's
slices masquerade as eligibility: the eligibility construction may read the frozen admissible surface
of §77.4 **interpretably** -- it is a rule about $c$, the unit and the unedited trajectory, and a reader
must be able to follow why a unit is in $E(c)$ -- while only the refinement is required to be a pure
field-semantic predicate of $u$ alone. Interpretability is also evaluated **after** eligibility, and the two classes carry **different**
rules, which is what the first draft conflated:

$$\boxed{\text{eligibility: a readable rule over } \bigl(c,\ u,\ W_{\text{pre}}\text{'s unedited
trajectory},\ \text{the frozen solve}\bigr)}$$

$$\boxed{\text{refinement: membership must be a pure field-semantic predicate of } u}$$

A set can be interpretable, stable and arm-blind and still not be unaffected -- `phase_even` was exactly
that -- so neither layer may substitute for the other, and the refusals below bind the slice $S_i$
**and** the eligibility predicate alike:$

Rejected by gate, not by taste:

* `hash(scene) % 2` and any arithmetic over a serialised unit;
* "the first $N$ units in enumeration order", or any index window;
* a mask derived from a measured $V$, from an arm's write target, or from the screening's outcome;
* a predicate that reads a field it does not name in its rule.

This is the modulo-adapter prohibition of B2-4, generalised from the context ontology to the scene one.

### 77.6 Coverage, statically

Coverage is decided by enumeration and never by measuring behaviour: the size of each candidate, its
complement's size, disjointness of the registered complementary pair
($C_{\text{even}} \cap C_{\text{odd}} = \varnothing$ and
$\lvert C_{\text{even}}\rvert + \lvert C_{\text{odd}}\rvert = \lvert E(c)\rvert$ -- the
partition is of the **eligibility layer**, not of $U_2$, since the slices are refinements of
$E(c)$ and $\lvert E(c)\rvert < 5760$ by the properness required in §77.4), and the per-$\kappa$ and per-$z_{\text{base}}$ breakdown. What is **not** decided
...
**The credited sites are frozen, at the credited-site granularity.** A89 §77.4's $c$ is not free:

$$\boxed{c^{\text{cal}}_{D_Q} = \texttt{DecisionAddress}(\texttt{State}(0,2,0,0,0), 1, 0), \qquad
c^{\text{cal}}_X = \texttt{ControllerSite}(\texttt{State}(1,1,2,0,0), 3), \qquad c^{\text{cal}}_P = 0}$$

The first is a **`DecisionAddress`, not a `QAddress`**: A76/A77 freeze
`owner_Q(QAddress) = DecisionAddress`, and the persisted scalar store sits one layer below the credited
decision context. Mixing the layers is not a notation slip -- two different address types are never
equal, so a $Q$-granular synthetic site would satisfy "off-target" automatically even when its row
belongs to the very decision context that was credited, and the intended site's own scalar would be
reported as collateral.

**The chain is deterministic at every arrow and runs before any measurement of that cell:**
...
1. **candidate.** $C$ is the registered refinement of §77.4 for that cell.
2. **unit.** With $\text{owner}_A$ the owner projection of $A$'s channel and $\mathcal I_A$ the edit
   function of step 4,

   $$\boxed{u^{\text{spill}}_{C,A} = \min_{\text{lex}} \bigl\{u \in C:\ \text{the } W_{\text{pre}}
   \text{ trajectory of } u \text{ consults an address } q \text{ of } A\text{'s channel with }
   \text{owner}_A(q) \neq c^{\text{cal}}_A \text{ and } \mathcal I_A(q) \neq \texttt{None}\bigr\}}$$
3. **site.** $q^{\text{spill}}$ is the lexicographically least such address, and

   $$\boxed{\text{owner}_A(\Delta W^{\text{spill}}_A) = s^{\text{spill}}_{C,A} :=
   \text{owner}_A(q^{\text{spill}}) \;\neq\; c^{\text{cal}}_A}$$

   The inequality is a **theorem at the credited granularity**, not a type accident: the selected scene is
   eligible, so it does not consult $c^{\text{cal}}_A$, while it consults $q^{\text{spill}}$ whose owner
   differs by selection. The gate asserts it in that form.
4. **edit.** $\mathcal I_A$ returns a unique legal edit or `None`, one clause per channel:

   * $D_Q$: with $d := s^{\text{spill}}_{C,D_Q} = \texttt{DecisionAddress}(s,z,m)$, and $a^F$ the command
     the unedited trajectory sends at $d$,

     $$\boxed{q^{edit} = \texttt{QAddress}(d.\text{state},\ d.z,\ d.m,\ a^F)}, \qquad
     v' = \min_{a \in A_z(m,s)} Q^{*}(s,z,m,a) - 1, \qquad
     \mathcal I_{D_Q} = \texttt{Edit}(Q,\ q^{edit},\ v')$$

     with $\texttt{owner}_Q(q^{edit}) = d$ required, and $\mathcal I_{D_Q} = \texttt{None}$ where
     $\lvert A_z(m,s)\rvert < 2$. The credited site, the store key and the effective value are three
     layers, and the edit is written at the store layer: leaving the key to inference is how a credited
     `DecisionAddress` becomes an arbitrary `QAddress`;
   * $X$: at $s^{\text{spill}}_X = \texttt{ControllerSite}(s, a^{\text{cmd}})$, the target is the least
     common admissible alternative
     $a' := \min\bigl(\bigcap_{z,m} A_z(m,s) \setminus \{a^{\text{cmd}}\}\bigr)$; `None` if that set is
     empty. The intersection is A88 §76.2's contract-preservation rule: a target legal at one witness is
     not automatically legal wherever the site fires;
   * $P$: at the proposal $z^{\text{proposal}} = s^{\text{spill}}_P$, the target is
     $z' := \min\bigl(\mathcal Z \setminus \{z^{\text{proposal}}\}\bigr)$, i.e.
     `Edit(PROCESS, z^{proposal}, z')`; `None` when $\mathcal Z$ has a single element.
5. **calibration reference, on the candidate rather than at the witness.** A unit-level sign cannot
   certify a set-level ruler: `BehavioralCollateral` is a **mean over $C$**, and one persistent edit
   moves more than one unit -- this screening's own canaries moved $120$, $120$ and $1440$ scenes -- so

   $$\boxed{\operatorname{sign}\Delta V(u^{\text{spill}}) \;\not\Longrightarrow\;
   \operatorname{sign}\texttt{BehavioralCollateral}(C)}$$

   The reference is therefore the **same functional**, evaluated on frozen predictions:

   $$\boxed{B^{ref}_{C,A} := \frac{1}{\lvert C\rvert} \sum_{u \in C}
   \bigl[V^{ref}_{\text{pre}}(u) - V^{ref}_{\text{spill}}(u)\bigr]}, \qquad
   d^{ref}_{C,A} := \operatorname{sign} B^{ref}_{C,A}$$

   with $V^{ref}_{\text{spill}}(u)$ formed as §75.7 forms $\Delta V^{ref}$: the frozen solver's rows with
   the edited address applied where the unit's trajectory consults it, and the unedited value where it
   does not. Calibration requires

   $$\boxed{\texttt{BehavioralCollateral}^{meas}_{C,A} \;=\; B^{ref}_{C,A}}$$

   within §75.6 G5's frozen tolerance -- **equality with a prediction, not merely non-vanishing** -- and
   $B^{ref}_{C,A} = 0$ is `CALIBRATION_FAIL`, because an injection the aggregate ruler predicts as
   invisible is not a calibratable spillover. The canary's $d_A$ is not inherited: $\mathcal I_A$ is a new
   edit at a new site, and nothing frozen implies equal signs.
6. **fail closed.** If no unit qualifies at step 2, the outcome is

   $$\boxed{\texttt{CALIBRATION\_FAIL}}$$

   reported as such: no relocated site, no alternative target, no narrowed domain.
...
* **no free magnitude or sign.** Neither is a parameter: the edit is $\mathcal I_A$'s, and the
  requirement is $\texttt{BehavioralCollateral}^{meas}_{C,A} = B^{ref}_{C,A}$ within tolerance, with the
  sign $d^{ref}_{C,A}$;
* **fail closed.** If no unit of $C$ consults any address of $A$'s channel -- an eligible region
  that never touches that channel -- the outcome is
...
The compatibility closure re-types the affected objects onto the scene unit:

* `collateral.py`: `EvaluationSceneSet` in place of `UnaffectedSet.contexts`, eligibility as
$E(c)$ of §77.4 and a refinement as a field-semantic slice of it, and
  `behavioral_collateral` summing $V_{\text{unaffected,pre}} - V_{\text{unaffected,post}}$ over the
  same set instance;
* `runner.py`: the pre-update source and the touched-unit bookkeeping re-typed to scenes; the
  decision-context plumbing stays where it belongs, with the decision-context candidates;
* invariants that survive the re-typing unchanged: one fixed nominal reference artifact for every cell
  and both sides; identical scenes for pre and post; units pre-constructed and arm-blind; no index,
  hash, modulo or resampling adapter standing in for a measurement.

### 77.9 Gates, including mutation power

The obligations are grouped by the contract each certifies, and every group must be shown able to
**fail**: a gate that cannot go red is not a gate.

**Foundation.** Typing and totality of membership; closed, non-empty, proper and duplicate-free
enumeration; truth-blindness through the existing AST/import layer; determinism (same inputs, same set,
no iteration-order dependence); the interpretability refusals of §77.5, one class per layer; the
complementary pair inside $E(c)$, $C_{\text{even}} \cap C_{\text{odd}} = \varnothing$ and
$\lvert C_{\text{even}}\rvert + \lvert C_{\text{odd}}\rvert = \lvert E(c)\rvert$; one shared reference
with no per-arm switching; and exactly one eligibility construction registered. Mutations that must
redden: a constant-membership candidate, a rule reading a measured value, an arm-dependent rule, a
hidden index window, an overlapping complement.

**Gate A -- aggregate reference calibration, with its provenance.** The measured side must equal the
predicted functional,

$$\boxed{\texttt{BehavioralCollateral}^{meas}_{C,A} \;\approx\; B^{ref}_{C,A}}$$

within §75.6 G5's tolerance -- not merely non-zero, and not merely a sign agreement. Four hostile
mutations must each turn it red: substituting the local loss at $u^{\text{spill}}$ for the expected
value; comparing signs only; requiring non-vanishing only; and post-processing the measured map to reach
the expected number. Its **provenance** is part of the gate rather than an assumption about the
implementer:

$$\boxed{B^{ref}_{C,A} \text{ may depend only on the frozen reference artifact and solve, on } C,
\text{ and on } \Delta W^{\text{spill}}_A}$$

so it may not read the measured pre or post maps, the measured $\texttt{BehavioralCollateral}$, any arm's
output, or the calibration result. Otherwise the equality certifies itself: an implementation that set
$B^{ref}_{C,A} := \texttt{BehavioralCollateral}^{meas}_{C,A}$, or
$V^{ref}_{\text{spill}} := V^{meas}_{\text{post}}$, would make `measured = expected` true by
construction while the reference prediction stopped being a prediction -- the discipline that keeps a
target from being the thing it judges. A fifth hostile mutation freezes it: **`expected := measured` must
turn the gate red**.

**Gate B -- the $D_Q$ store-key and owner chain.** $q^{edit}$ must be an exact `QAddress`,
$\texttt{owner}_Q(q^{edit}) = s^{\text{spill}}_{C,D_Q}$ must hold, and the edit must travel through the
substrate's transaction. Mutations that must redden: a `DecisionAddress` passed where a `QAddress` is
required; the same row's other action edited; an owner that does not project onto the synthetic credited
site.

**Gate C -- production credited-domain completeness.** The three extents of §77.4 are enumerated in
full, deterministically and without duplicates, with no legal production credit omitted and none added,
and $X$'s entries pass **both** validators. Mutation: dropping one credited site from the enumerator
must redden -- otherwise "$\forall c$" could be quietly narrowed by an incomplete enumerator.

**Gate D -- refinement-level totality.** $\forall A, c, i:\ C_i(c) \neq \varnothing$ is executed, not
inferred from $E(c) \neq \varnothing$. Mutation: a refinement accepting only a phase or base option that
no unit has -- empty at some legitimate $c$ -- must fail at admissibility rather than wait for the
development coverage question.

**Bookkeeping probe.** The summary row is asserted against the table's column count: a raw `|` inside a
formula opens a column while leaving the row on a single physical line, so "it is one line" is not the
property that matters.

**Gate E -- calibration chain integrity.** The chain of §77.7 is deterministic at every arrow;
$s^{\text{spill}}_{C,A} \neq c^{\text{cal}}_A$ holds for every cell at the credited granularity;
$\mathcal I_A$ is the only source of a target; and an unreachable injection yields `CALIBRATION_FAIL`
rather than a relocated fixture or a narrowed domain. Mutations that must redden, one per failure mode:
taking the next site when the canonical one is unavailable; accepting a caller-supplied target and thereby
bypassing $\mathcal I_A$; and narrowing $C$ whenever $\mathcal I_A$ returns `None` and continuing with the
smaller candidate.
...
| shared scenes and CRN | **PASS at the measurement interface** |
| closed nominal map | **PASS** ($5760$ units, missing/extra and non-finite refused, no short-circuit) |
| load-bearing for $D_Q$, $X$, $P$ | **PASS for $U_2$** (the six-cell matrix) |
| synthetic spillover in the unaffected region | **PASS for the six registered refinements** (18/18 seedless calibration cells) |

PASS now means the seedless Collateral instrument is executable on the surviving $U_2$ ontology and on
all six registered unaffected-region constructions: raw-pre/own-post measurement, shared scenes, the
closed nominal map, three-architecture load-bearing behaviour, and synthetic spillover calibration have
all been exercised. What remains open is **which refinement becomes the development-selected primary**;
instrument closure does not select one.

### 77.11 Boundary

A89 remains **FROZEN at `rebuild@6c28fbc`**. Its authorised seedless compatibility and calibration
implementation is now **CLOSED at implementation revision `a4451cc`**: Foundation and Gates A--E are
exercised, the production runner is re-typed onto the scene ontology, the actual B1 credited unit is
bound to the region's $c$, one reference artifact is shared by identity, and $D_Q$, $X$ and $P$ are all
reachable through their existing B1 entry points.

This closure is **instrument closure only**. It does not itself authorise a development seed or select a
refinement. Still **NOT AUTHORISED**: choosing a construction on measured effects, $N_{\text{eval}}$,
$\mathcal G_{\text{ckpt}}$, the Retention form, $T$, $\Delta_{\min}$, $N_{\text{train}}$, smoke-5,
development-32, the confirmatory stage, or V0.4R.
...
**Round 2 (`e77a0e8`).** Four defects, all in §77.4--§77.7. The eligibility layer carried only the
*unrelated* conjunct, and its second variant evaluated *faithful at $c$* on trajectories that never
consult $c$ -- undefined rather than stricter; both are replaced by one scene-level definition
$E(c) = \{u: \neg\textit{Consult}(u,c) \wedge H_{\text{pre}}(u)\}$, with $H_{\text{pre}}$ the unedited
episode's own healthy outcome. Keeping two eligibility constructions also contradicted the obligation
count, since refinements are intersections: the eligibility is now single, so the registered set is six
constructions and $6 \times 3 = 18$ obligations. Calibration was self-defeating as written -- it looked
inside $C$ for a scene consulting the very address eligibility excludes -- so the credited site and the
synthetic site are now separate objects with a proved inequality between them. And §77.6 still carried
the first draft's identity $\lvert C_{\text{even}}\rvert + \lvert C_{\text{odd}}\rvert = \lvert U_2\rvert$,
which is false once the slices are refinements of $E(c)$. Interpretability is split into the eligibility
class, which may interpretably read the frozen admissible surface, and the refinement class, which must
be a pure field-semantic predicate of the unit.

**Round 3 (`7d8b1bd`).** The eligibility layer and the six-refinement arithmetic were accepted; five
defects remained in the calibration interface. $c^{\text{cal}}_A$ had a role but no value or rule, so the
eighteen cells could be reshaped by choosing the credited site; it is now frozen at credited-site
granularity, with the $D_Q$ entry a `DecisionAddress` rather than a `QAddress`, because the wrong layer
makes the off-target test vacuous by type inequality alone. The synthetic site and the edit were named
but not defined, so the canary's behaviour could not be transported; $\mathcal I_A$ is now a function
returning a unique legal edit or `None`, per channel, with the intersection rule A88 §76.2 established
for controller targets. The expected sign was inherited from the canary, which nothing implies at a
different site; $d^{\text{spill}}_{C,A}$ is now an independent frozen prediction. The construction
contract was not quantified over production credited units; it now requires $E(c) \neq \varnothing$ and
$E(c) \subsetneq U_2$ for every $c \in \mathcal D^{\text{credit}}_A$, with calibration fixtures kept a
separate obligation. Three stale passages still described eligibility as a deferred choice or a
field-semantic predicate, contradicting §77.4's single frozen construction; §77.4, §77.5 and §77.8 are
corrected, and §77.5 now carries one rule per interpretability class.

**Round 4 (`e6bfb46`).** The calibration chain was complete but certified the wrong object: the predicted
direction was a property of one unit while `BehavioralCollateral` is a mean over the candidate, and one
persistent edit moves many units, so the two signs need not agree -- the reference is now the same
functional on frozen predictions, $B^{ref}_{C,A}$, checked for equality within tolerance rather than for
non-vanishing. Three further closures: $\mathcal I_{D_Q}$ is written through its three layers with an
explicit `QAddress` and an `owner_Q` requirement instead of an inferred key; the credited domains gained
executable extents, with $X$ needing both its domain filter and `require_admissible_sites` because
`X_DOMAIN` alone is not the credited population; and totality is required per refinement, since an empty
$C_i(c)$ has no mean, so definedness is seedless admissibility while coverage quality stays a
development question. Two stale passages were cleared: the summary row no longer offers a choice among
eligibility constructions, and §77.4 now separates provenance from value for the canary's site.

**Round 5 (`41596e8`).** Scientific and interface content approved. The formal freeze is withheld only for
a synchronisation: §77.9 still listed the older gate set, so the four contracts v5 introduced --
aggregate reference calibration, the $D_Q$ store-key and owner chain, production credited-domain
completeness, and refinement-level totality -- are now gate families with named hostile mutations, and
the summary row no longer describes a unit-level prediction with a non-vanishing test.

**Round 6 (`7239b27`).** The closure patch's three defects were all in the patch. The summary row had
been rejoined onto one line but still carried three raw delimiters -- two from $\frac{1}{|C|}$ and one
spurious separator -- so it parsed as eight columns against the A88 row's five; the row now uses
$\lvert C\rvert$ and asserts its delimiter count. Gate A lacked the provenance contract that stops the
reference from being defined by the measurement it judges, and Gate E, alone among the families, named no
hostile mutation. All three are closed.


**Freeze and promotion.** Review passed on `6c28fbc`: the scientific/interface content approved at
`41596e8` is unchanged, closure patch 1 (`7239b27`) and closure patch 2 (`6c28fbc`) complete the
required gate/mutation-power and bookkeeping synchronisation, and the branch is a strict fast-forward
from `rebuild@4472ec4` with only `docs/rebuild/12-AMENDMENTS.md` changed. Therefore:

$\boxed{\text{A89 = FROZEN at rebuild@6c28fbc}}$

$\boxed{\text{seedless compatibility + calibration implementation = AUTHORISED}}$

The implementation must close Foundation and Gates A--E, including their hostile mutations, before any
development seed can be used. B2 scientific seeds, smoke-5, development-32, confirmatory work and V0.4R
remain **NOT AUTHORISED**.


**Seedless implementation closure.** The implementation line
`7543bdb -> 1ab696e -> a63ccf1 -> bd88b92 -> a9469ed -> 69b6e29 -> a4451cc` was reviewed as a
strict fast-forward implementation of the frozen A89 contract. The final revision closes the production
bindings that the calibration-only path could not prove: one raw $W_{pre}$ trace source; one shared
pre-measurement object; per-arm raw-pre-to-own-post Collateral; exact binding between the region's
credited unit and B1's resolved credit; one reference artifact by identity; command-keyed $X$ consult
semantics; and a load-bearing $D_Q$ dispatch through the existing B1 entry point.

Evidence at `a4451cc`: **694 tests, exit 0**; `spec_audit` exit 0; mutation table **45/45
`GATE_IS_REAL`** with zero harness/node failures and byte-identical restoration; calibration artifact
**18/18** with no failures and worst absolute gap
$4.440892098500626\times 10^{-16}$.

$\boxed{\text{A89 seedless compatibility + calibration implementation = CLOSED at a4451cc}}$

No scientific seed is authorised by this bookkeeping closure.

## 78. A90 — the development time axis: protocol freeze, smoke, design lock, confirmatory

**Status: FROZEN at `rebuild@b6ee60e`.** A90 itself authorises no seed. It does not reopen A89's
ontology, calibration or compatibility closure, and it cannot retroactively legitimise a run that already
happened.
...
A89 closed the instrument. A79's stage order is already frozen and is **not** a single gate:

$$\boxed{\texttt{smoke5} \to \texttt{dev32} \to \texttt{lock} \to \texttt{confirmatory}}$$

and A79 §67.9 lets development data inform $T$, $\mathcal G_{\text{ckpt}}$, $N_{\text{eval}}$ and the
Retention form. $T$ makes the point sharply: $T = Q_{0.9}(T_{\text{conv}})(1 + h)$ with $h = 0.20$ is
computed from **development baseline seeds**, so

$$\boxed{T\text{'s value cannot exist before the first development seed}}$$

while what must exist before that seed is the rule that produces it. One freeze therefore cannot carry
both obligations.

### 78.2 The state machine

$$\boxed{
\begin{aligned}
S_0 &: \text{A89 instrument CLOSED; no seed drawn}\\
...
\text{refinement},\ \text{Retention},\ \{\Delta_{\min,e}\}_{e \in \mathcal E_\Delta}\\
...
Each arrow is a gate, and each state names what may be read. The load-bearing inequality is

$$\boxed{F_0 \;\neq\; F_1}$$

because $F_0$ freezes rules and $F_1$ records what those rules compute.

### 78.3 $F_0$: `DEV_PROTOCOL_FREEZE`, before any smoke or development seed
...
and the fingerprints of the artifacts matching the ones $F_0$ recorded. No criterion may depend on a
treatment effect: smoke decides whether the instrument runs, never whether an effect is present.

**Invalidation.** If smoke leads to any change in code, configuration or instrument, then

$$\boxed{\text{the old fingerprint is invalid} \;\Rightarrow\; \text{re-close the gates}
\;\Rightarrow\; \text{re-freeze the protocol} \;\Rightarrow\; \text{re-run smoke}}$$

A repaired instrument may not inherit the old smoke PASS.

### 78.5 $S_2 \to F_1 \to S_3$: the design lock, and the data-access order
...
f_T, f_N, f_G, f_C, f_R \;\text{are evaluated on it}$$
...
The refinement and the Retention form are chosen by **different rules with different input surfaces**,
and A90 keeps them apart:

$$f_C:\ \text{refinement} \;\longrightarrow\; \text{one of the six, or }
\texttt{NO\_ADMISSIBLE\_REFINEMENT}$$

from A79 §67.4's properties -- coverage, baseline stability, interpretability, sensitivity to synthetic
spillover;

$$f_R:\ \text{Retention form} \;\longrightarrow\; \text{one registered form, or }
\texttt{NO\_ADMISSIBLE\_RETENTION}$$
...
$$\boxed{\text{gate} \quad|\quad \text{static descriptor} \quad|\quad \text{dev-estimated ranking
metric}}$$

**Synthetic spillover sensitivity is a gate, never a ranking metric.** A89 froze that calibration
magnitudes may not rank candidates, and $18/18$ calibration PASS is what it licenses: all six
refinements are measurable. Neither $\lvert B^{ref}\rvert$ nor any measured calibration magnitude may
appear in a ranking functional. Coverage and interpretability are likewise not automatically continuous
"larger is better" numbers: A90 requires each property's role to be declared in $F_0$, so that a
threshold-like property enters as a gate and a descriptive one as a descriptor.

### 78.7 Fail-closed codomains, and the branch they keep open

A77/A86 keep the architecture-specific branch enterable when every surviving unified candidate fails
A85's other properties. A total selector onto one candidate would contradict that:

$$\boxed{\text{no admissible refinement} \;\Rightarrow\; \text{no primary refinement}
\;\Rightarrow\; \text{no confirmatory stage}}$$

and the path onward is a new amendment entering $U_3$, not a tie-break among failures. Tie-breaks happen
only among candidates that all meet the declared minimum admissibility, and the tie-break rule itself is
frozen in $F_0$. The Retention selector is fail-closed in the same way.

### 78.8 Supersession: no candidate expansion after the first seed

$$\boxed{\text{A90 supersedes A79 §67.5 on post-first-seed candidate expansion}}$$

Once the first smoke or development seed exists, no Retention form (and no refinement) may be added to a
candidate set. Adding one requires a new amendment, a new $F_0$, and a **fresh** development seed set;
the old development data may not be reused to select among the enlarged set. Without this, the freeze
would carry both "the candidate set is immutable" and "development may add forms".

### 78.9 $N_{\text{train}}$ does not exist, and is not named here

$$\boxed{N_{\text{train}} \;\notin\; \text{A90's design freeze}}, \qquad
\boxed{N_{\text{train}} = \text{NONEXISTENT / OPEN}}$$

A79 §67.10 and §67.12 say it does not exist yet and may not be named in the pre-registration. The design
freeze declares no value for it, and a future training protocol would need its own amendment. An earlier
...
**Round 5 (`b6ee60e`).** Formal content approved. The endpoint universe is now a registry of
threshold-bearing **statistical endpoints**, not dimension labels: RMST and DeficitAUC are necessarily
distinct members, completeness is checked against every confirmatory verdict/bound that uses
$\Delta_{\min}$, and every registry member has exactly one frozen threshold provenance. The final
implementation-time choice is also closed: $F_0$ records an acyclic $\mathcal D_{F_1}$ and the joint
$F_1$ lock evaluates the design rules first and the threshold rules second inside one commit. No
scientific seed is authorised by this approval.
...
**Freeze and promotion.** Review passed on `b6ee60e`. The A90 state machine, authorisation boundary,
pre-lock information surface, master acquisition envelope, complete design-rule family, endpoint registry,
threshold provenance and $F_1$ dependency graph are accepted as the frozen development-stage protocol.

$\boxed{\text{A90 = FROZEN at rebuild@b6ee60e}}$

$\boxed{\text{next authorised action = construct and review }F_0\text{ only}}$

A valid $F_0$ is still required before `smoke5`; therefore smoke, development, confirmatory seeds and
V0.4R remain **NOT AUTHORISED** at this freeze.
...
| **A91** | **the B2 training-time and episode-axis contract (DRAFT FOR REVIEW)**: the review of $F_0$ rev 2 ruled that its twenty-first item cannot be ratified inside a protocol freeze, because the mechanism that indexes a baseline curve's episodes is not a parameter but the axis `05`'s convergence rule, A84's quadrature and the whole FutureUtility family presuppose --- and A90 §78.9 left it open on purpose ($N_{\text{train}}$ = NONEXISTENT/OPEN, a future training protocol needing its own amendment). The gap is a type error: `05` §2 defines $N_{\text{train}}$ as training episodes **per run**, `05` §6.1 reads convergence per development seed off the `NoCorrection` arm, `05` §6.2 fixes a dense-early grid and integration on the real episode index, and A84 §72 froze trapezoidal quadrature with $episodes[0]=0$, $episodes[-1]=T_{\max}$ --- while B2's production path reads the axis off an array, `_episode_grid(levels) = tuple(range(len(levels)))`, so "episode $t$" is a position inside one kernel rollout and $T_{\max}$ could never exceed `HORIZON`$=12$. A91 freezes the missing layer: a training episode as $W_e \to$ rollout under $\tau_e \to$ update $\to W_{e+1}$; persistence of exactly the learner's substrate and nothing else; an in-place tabular $Q$-learning update through the existing edit vocabulary with arm-invariant constants and keyed exploration draws (common random numbers by coordinate, not by consumption order); one seed generating its whole run through a frozen key derivation $\tau_e = \texttt{SemanticTape.sample}(k(s,e))$, shared by both arms; read-only checkpoint evaluation that may write nothing and consume no stream element; real-axis definitions of $V(e)$, $\mathrm{slope}_K$, $\mathrm{FlipRate}_K$ and a right-censored, never-dropped $T_{\text{conv}}$; per-seed values `restricted_time`/`DeficitAUC` for the redundancy input, because RMST is a population summary; and replacement of the step-indexed producer while the endpoint arithmetic stays as A84 froze it. Marks its own proposals `[PROPOSED]`, names no seed, no $N_{\text{train}}$, no $T$, no $\alpha,\gamma,\epsilon_s,\epsilon_f$ value and no grid value, and authorises no code and no seed until frozen. | **P0 (time axis)** | **DRAFT FOR REVIEW --- not frozen, authorises nothing** |
...
## 79. A91 --- the B2 training-time and episode-axis contract (DRAFT FOR REVIEW)

$$\boxed{\textbf{DRAFT FOR REVIEW --- not frozen, authorises nothing, draws no seed}}$$

$$\boxed{\text{kernel step index} \;\neq\; \text{training episode index}}$$

### 79.1 The gap is a type error, not a missing constant

$F_0$'s eleventh checklist item asked which mechanism indexes a baseline curve's episodes in this rebuild
line, and could not be ratified inside a protocol freeze. It is not a parameter: it is the axis that
`05`'s convergence rule, A84's quadrature and the whole FutureUtility family presuppose, and A90 §78.9
left it open on purpose ---

$$\boxed{N_{\text{train}} = \text{NONEXISTENT/OPEN}, \qquad
\text{a future training protocol needs its own amendment}}$$

The frozen side is unambiguous about what the axis *is*:

* `05` §2 defines $N_{\text{train}}$ as **training episodes per run** --- per run, not per step, and not
  per evaluation scene;
* `05` §6.1 fixes baseline convergence **per development seed on the `NoCorrection`/baseline arm**, as the
  first checkpoint after which, for $K$ consecutive checkpoints,
  $\lvert \mathrm{slope}_K \rvert < \epsilon_s$ **and** $\mathrm{FlipRate}_K < \epsilon_f$, giving
  $T^{(i)}_{\text{conv}}$ and then $T = Q_{0.9}(T_{\text{conv}}) \times (1+h)$;
* `05` §6.2 fixes a **dense-early** grid and requires every time-integrated endpoint to be computed by
  integrating over the **real episode index** rather than by averaging checkpoint values;
* `05` §§8.1--8.3 define $\tau$, RMST and DeficitAUC on that same axis, and A84 §72 froze their numerical
  realisation as **trapezoidal integration on the real episode indices**, with $episodes[0] = 0$,
  $episodes[-1] = T_{\max}$, a short curve **refused rather than padded**, and left-hold explicitly
  rejected because "the measurement at $t_i$ persists across $[t_i, t_{i+1}]$" is an assumption `05` never
  makes.

The implementation side is unambiguous about what it currently uses. B2's production path reads a curve's
episode indices off the array it happens to hold:

```python
def _episode_grid(levels) -> tuple:
    return tuple(range(len(levels)))          # runner.py:422
```

so "episode $t$" is a **position inside one kernel rollout**. With `HORIZON = 12` that path cannot
represent an episode index above $11$ at all: $T_{\max}$ would be a step count, `05` §6.2's dense-early
grid would be a 12-element prefix, `1/T_{\max}` in DeficitAUC would normalise by a rollout length, and the
$K = 3$ maintained-recovery window would be three *steps* of one episode rather than three checkpoints of a
training run. Four frozen rules would each be silently re-parameterised by one loop.

**A91 is therefore the amendment A90 anticipated**, not a patch to $F_0$. It freezes the training-time
substrate `05` assumes and A90 left unnamed.

### 79.2 What a training episode is

$$\boxed{\text{a run} = \text{one learner's sequence of training episodes } e = 0, 1, 2, \ldots}$$

A **training episode** $e$ is one traversal of the benchmark by the learner's current state, followed by
that learner's update:

$$W_e \;\xrightarrow[\text{rollout under } \tau_e]{\text{episode } e}\; \text{transitions, rewards,
outcome} \;\xrightarrow[\text{the update rule of §79.4}]{\text{after the episode}}\; W_{e+1}$$

* $W_e$ is the learner's **persistent state** at the start of episode $e$ --- the substrate B1's write
  mechanism mutates and nothing else;
* the episode's rollout is one pass of the frozen kernel (§65's `State`, `ControlState`, tape, option
  automata) under the episode's tape $\tau_e$ and the learner's current decision policy;
* the **episode axis** is the integer index $e$, and the endpoints read *these* indices. The kernel's
  within-episode step index $t = 0 \ldots H$ is a different quantity and may never be used as $e$:
...
**The level of an episode.** The curve's value at $e$ is the **evaluation-sample mean of the frozen
performance measure on $W_e$**:

$$\boxed{V(e) = \frac{1}{\lvert \mathcal S_{\text{eval}}(N_{\text{eval}}) \rvert}
\sum_{u \in \mathcal S_{\text{eval}}(N_{\text{eval}})} m(W_e; u)}$$

with $\mathcal S_{\text{eval}}$ the master evaluation sample and $m$ the same measurement operator A85
§73.1 fixed for $V_{\text{pre}}$ --- so that $V(\cdot)$ evaluated at $W_e = Q^{*}$ *is* $V_{\text{pre}}$ and
the two quantities cannot drift apart into two different instruments. The sample is a measurement operator:
its units are pre-constructed and arm-blind (A86 §74.5), and it is read, never trained.

### 79.3 Persistence: what crosses an episode boundary

$$\boxed{\text{what persists is } W_e \text{; the tape, the rollout and every measurement do not}}$$

* **inherited**: the learner's persistent substrate and its ledger --- exactly what the update rule
  writes. Episode $e+1$ begins from the state episode $e$ produced, not from a re-minted initial state;
* **not inherited**: the tape $\tau_e$ (a function of the seed and $e$, §79.5), the transient rollout
  state, and any measured value. A checkpoint measurement may not leak into $W$ (§79.6);
* **initial condition**: episode $0$ begins at the arm's frozen initial state --- $W_{\text{pre}}$ for the
  reference arm, and $W_{\text{pre}}$ with the B1 write applied for the treatment arm. Nothing else
  distinguishes the arms at $e = 0$.

### 79.4 The update rule

A91 must freeze that the learner **learns**, because `05` §6.1's convergence rule is a statement about a
learning curve: a fixed post-write state repeated $N_{\text{train}}$ times has no convergence time, and
`NoCorrection` names the absence of a *correction write*, not the absence of learning.

$$\boxed{\text{form: in-place, tabular, off-policy } Q\text{-learning on the episode's own transitions}}$$

* the update consumes the episode's learner-visible transitions, the observed rewards and the frozen
  reference view (read-only, never written), and writes through the **existing** substrate edit vocabulary
  (`apply_transaction`) rather than a new store: the training path and the B1 path mutate one substrate,
  which is what makes a post-write learner and a trained learner comparable at all;
* it is a **pure function** of $(W_e, \text{episode trace}, \alpha, \gamma)$ --- no unkeyed randomness ---
  so that a seed's whole run is reproducible from the seed;
* **exploration** must exist, and its randomness must come from the keyed stream of §79.5, drawn by
  $(e, \text{step})$ rather than by consumption order, so that two arms whose policies diverge still draw
  the *same* variates at the same coordinates (common random numbers, not common prefixes);
* $\alpha$, $\gamma$ and the exploration schedule are **design quantities**: A91 freezes the form and
  leaves their values to the design lock, because `05` §2's list permits the development stage to inform
  runtime and stability and nothing more.

**`[PROPOSED]`** --- the rule's form, its in-place tabular character, the keyed-draw discipline and the
"same rule and constants for every arm" clause are this draft's proposal and need ratification. The
alternative --- a within-episode, step-indexed learner --- is exactly what §79.1 refutes.

### 79.5 One seed, one cross-episode stream

$$\boxed{\tau_e = \texttt{SemanticTape.sample}\bigl(k(s, e)\bigr), \qquad
\text{exploration variates} = \rho\bigl(k(s, e), i\bigr)}$$

* $s$ is the statistical seed and $k$ a **frozen key derivation** from $(s, e)$; the tape is therefore a
  deterministic function of the pair, and one seed generates a whole run rather than one episode;
* $\rho$ is keyed by $(e, i)$ --- episode and within-episode coordinate --- so alignment does not depend
  on how many variates an arm's policy happened to consume;
* **paired arms share the stream**: for every $e$, both arms of a pair see $\tau_e$ and the same variates
  at the same coordinates. This is the training-time analogue of A85's shared measurement draw, and it is
  what makes a between-arm difference attributable to the initial write;
* the evaluation at a checkpoint uses the same shared sample, so measurement is CRN-paired as well;
* distinct seeds give independent runs; a run is a deterministic function of its seed alone.

**`[PROPOSED]`** --- the concrete $k$ and $\rho$ (an affine mixing of $(s, e)$ in 64-bit arithmetic is one
candidate) are declared in A91's implementation step and are not chosen here.

### 79.6 Checkpoint evaluation is read-only

$$\boxed{\text{the evaluator reads } (W_t, \mathcal S_{\text{eval}}) \to V(t) \text{ and may write
nothing}}$$

It may not update $W$, consume the training stream, advance the episode counter, or mutate the reference;
$V(t)$ is a function of the training path, not a second training process. This is gateable rather than
merely stated: the learner's digest is compared before and after an evaluation, and the stream's position
is asserted unchanged. A checkpoint that trained the learner would make the curve a function of the
measurement schedule, which is the same class of error as reading a recovery target off the arm's own
result (A85 §73.1).

### 79.7 The frozen quantities, now on a real axis

* **$V(e)$** --- §79.2's evaluation-sample mean;
* **$\mathrm{slope}_K$** --- on the real, non-uniform grid, the per-episode rate over the last $K$
  checkpoints,
  $\mathrm{slope}_K(j) = \dfrac{V(t_j) - V(t_{j-K})}{t_j - t_{j-K}}$
  (an index-difference denominator would reintroduce the non-uniformity `05` §6.2 warns about);
* **$\mathrm{FlipRate}_K$** --- the fraction of adjacent increments in the last $K$ intervals whose
  direction reverses: with $\delta_i = V(t_{i+1}) - V(t_i)$,
  $\mathrm{FlipRate}_K(j) = \#\{i : \delta_i\,\delta_{i-1} < 0\} \big/ \#\{i\}$ over the last $K-1$
  adjacent pairs. `[PROPOSED]` **zero increments**: $\delta = 0$ is neither a reversal nor a
  continuation, so a reversal requires two nonzero increments of opposite sign --- declared rather than
  left to a sign convention;
* **$T_{\text{conv}}$** --- per development seed, on the baseline arm only, the frozen `05` §6.1
  condition read for $K$ consecutive checkpoints; $T^{(i)}_{\text{conv}}$ is the **last checkpoint of the
  first qualifying window**. A seed that never qualifies is **right-censored at the acquisition cap and
  reported as censored**, never dropped, because $Q_{0.9}$ computed on an uncensored subset is a
  different statistic from the one `05` §6.1 names;
* **$\epsilon_s, \epsilon_f$** --- `[PROPOSED]` declared constants of A91's implementation step; without
  them the convergence rule is not executable and $T$ cannot be computed;
* **$\tau$, `restricted_time`, RMST, DeficitAUC** --- unchanged from `05` §§8.1--8.3 and A84, now fed by an
  axis that exists. The per-seed quantity remains `restricted_time`; RMST remains the cross-seed
  expectation and belongs to B2-4 (A84 §72's naming rule);
* **aggregation** --- $V(t)$ values are within-seed measurements, so `05` §7 applies unchanged: any test's
  independent observations are the **seeds**, and a mechanical assertion of that count is required.

### 79.8 What the arms share after the initial write

$$\boxed{\text{same stream, same episode count, same update rule and constants, same checkpoints, same
evaluation sample, same horizon}}$$

The only difference between a pair's arms is the initial B1 write. Everything downstream of it is shared
protocol, which is what `05` §6.1's baseline-only rule assumes when it reads convergence off the
`NoCorrection` arm and then applies the same horizon to both.

**And the execution boundary is untouched**: sharing a *mechanism* is not authorising a *run*. Until $F_1$
locks $T$, $N_{\text{eval}}$, the grid and the form, only the baseline arm may be executed (A90 §78.5),
and no treatment-arm curve may be plotted, printed or summarised (`05` §6.1, A79 §67.10).

### 79.9 Replacing the step-indexed production path

`FutureUtility.from_curve(values, episodes, *, pre_level, t_max)` already takes the episode indices, and
`retention.*.value(values, episodes, ...)` already validates them, so A91 changes the **producer** rather
than the endpoint arithmetic:

$$\boxed{\texttt{runner.py:422} \;\longrightarrow\; \text{the training loop of §§79.2--79.5}}$$

* the runner's curve is produced by the episode loop, indexed by real episode numbers, and the grid is
  $\mathcal G_{\text{ckpt}}$;
* a call whose `episodes` is an array-position range is a **contract violation** and is refused, not
  reinterpreted: the refusal is the difference between deprecating a reading and silently keeping it;
* Retention's $H^{*}, H_1^{*}, H_2^{*}$ ($F_0$ §4.5) become well-typed as episode indices under this
  axis, which is one of the two reasons $F_0$ could not ratify them;
* $F_0$'s other blocked items follow the same pattern: the **grid family** must be a dense-early
  coarsening of `05` §6.2 with $0$ and $T$ (not the $T/2, T/3, T/4$ spacing of $F_0$ rev 2, which is
  sparse where recovery happens), the **master evaluation ordering** must be a pre-frozen balanced order
  rather than A88's canonical lexicographic prefix, and **$f_R$'s redundancy** must be computed across the
  development seeds' per-seed values $(\texttt{restricted\_time}_i, \mathrm{DeficitAUC}_i)$, because RMST
  is a population summary and no per-scene RMST series exists.

### 79.10 Boundary

$$\boxed{\text{A91 names no seed, no } N_{\text{train}}\text{, no } T\text{, no } \alpha, \gamma,
\epsilon_s, \epsilon_f \text{ value, no grid value, no } \Delta_{\min}}$$

* A79 §67.10 does not merely defer $N_{\text{train}}$; it forbids naming it in the pre-registration, and
  A91 respects that: A91 freezes the **mechanism** that makes $T$'s already-frozen rule executable, and
  the **number** is computed at the design lock from development baseline data, exactly as `05` §6.1 and
  A90 §78.5 require;
* A91 re-opens nothing in A79, A84, A85, A87, A89 or A90. It supplies the axis those instruments
  presupposed, and it supersedes one implementation habit: the step-indexed reading of `_episode_grid`;
* **it authorises no code and no seed until it is frozen.** The order is
  $\text{A91 frozen} \Rightarrow$ correct the future-curve instrument $\Rightarrow$ re-gate
  $\Rightarrow$ $F_0$ finalisation $\Rightarrow$ clean execution manifest $\Rightarrow$ $F_0$ VALID review
  $\Rightarrow$ only then `smoke5`. Implementation before the freeze would put a design choice inside a
  loop, which is the failure mode A83 §71.4 and A84 §72 both exist to prevent.

### 79.11 Review record

**Round 1 (this draft).** Written after the review of $F_0$ rev 2 (`c643b73`) failed the protocol freeze on
its twenty-first checklist item and ruled that the item cannot be ratified inside $F_0$: the axis is not a
free parameter but a missing layer, and the review named the eight things it must freeze --- what a training
episode is, how the `NoCorrection` baseline persists across episodes, how the learner updates after one,
how a seed generates the cross-episode stream, that checkpoint evaluation is read-only, the real
episode-axis definitions of $V$, $T_{\text{conv}}$, $\mathrm{slope}_K$ and $\mathrm{FlipRate}_K$, what the
arms share after the initial write, and how the step-indexed production path is replaced. This draft
answers all eight and marks its own proposals `[PROPOSED]` rather than presenting them as consequences of
frozen text.
